      ...
      <div className="px-6 mt-6">
        <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4">
          {['All', 'Badminton', 'Tennis', 'Football'].map((tab, i) => (
            <button key={tab} className={`px-5 py-2 rounded-full text-sm font-semibold whitespace-nowrap ${i === 0 ? 'bg-green-500 text-white' : 'bg-white border text-gray-600'}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {loading ? <Loader2 className="animate-spin mx-auto mt-10 text-gray-400" /> : (
          <div className="space-y-4 pb-4">
            <p className="text-sm text-gray-500">{bookings.length} games found</p>
            {bookings.map((booking) => {
              const court = courts[booking.CourtID] || {};
              const isJoined = user.Bookings?.includes(booking.id);

              return (
                <div key={booking.id} onClick={() => onSelectMatch({ ...booking, courtDetails: court })} className="bg-white p-3 rounded-2xl border shadow-sm flex gap-4 cursor-pointer">
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <span className="text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded font-bold uppercase">{booking.Status}</span>
                    </div>
                    <div className="text-gray-500 text-xs mt-1 flex items-center gap-1"><MapPin size={12}/> {court.location}</div>
                    <div className="text-gray-500 text-xs mt-1 flex items-center gap-1"><Calendar size={12}/> {booking.start}</div>
                    <div className="flex justify-between items-center mt-3">
                      <span className="text-orange-500 text-xs font-bold flex items-center gap-1"><Users size={12}/> {booking.Member?.length || 0} Players</span>
